...
angular.module('liangxin-poi.services', ['ngResource'])

.service('Poi', ['$resource', function($resource){
	var poi = $resource('../api/v1/post/:id', {id: '@id', type: '地图'}, {
		query: {method: 'GET', isArray: true, interceptor: {response: responseInterceptor}},
		create: {method: 'POST'},
		update: {method: 'PUT'}
	});
	
	poi.prototype.$save = function(a, b, c, d){
		if(this.id){
